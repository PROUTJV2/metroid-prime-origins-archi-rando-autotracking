...
metroid_prime_origins_location = {}
metroid_prime_origins_location.__index = metroid_prime_origins_location
...
---@type table<string, metroid_prime_origins_new_return>
NAMED_LOCATIONS = {}
local stale = true
...
---@class metroid_prime_origins_new_return
...
---@field name string
---@field side string?
---@field exits table<integer, {[1]:metroid_prime_origins_new_return, [2]:fun(): accessibilityLevel}>
---@field keys integer
...
---@param name string
---@return metroid_prime_origins_new_return
function metroid_prime_origins_location.new(name)
	if name == nil then
		error("metroid_prime_origins_location name cannot be nil")
	end
	if NAMED_LOCATIONS[name] then
		print("metroid_prime_origins_location " .. name .. " already exists")
	end
    local self = setmetatable({}, metroid_prime_origins_location)
	self.name = name
...
---@param exit string|metroid_prime_origins_new_return metroid_prime_origins_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function metroid_prime_origins_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = metroid_prime_origins_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
---@param exit string|metroid_prime_origins_new_return metroid_prime_origins_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function metroid_prime_origins_location:connect_two_ways(exit, rule)
...
---@param exit string|metroid_prime_origins_new_return metroid_prime_origins_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function metroid_prime_origins_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
---@param exit string|metroid_prime_origins_new_return metroid_prime_origins_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function metroid_prime_origins_location:connect_two_ways_entrance(name, exit, rule)
...
function metroid_prime_origins_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
---@param rule1? fun(): accessibilityLevel|boolean
---@param rule2? fun(): accessibilityLevel|boolean
function metroid_prime_origins_location:connect_two_ways_stuck(exit, rule1, rule2)
...
---@return accessibilityLevel
function metroid_prime_origins_location:accessibility()
...
---@param keys integer
function metroid_prime_origins_location:discover(accessibility, keys)
...
Entry_Point = metroid_prime_origins_location.new("Entry_Point")
